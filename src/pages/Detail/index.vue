        ...
        <div class="InfoWrap">
          ...
          <div class="choose">
            ...
            <div class="cartWrap">
              ...
              <div class="add">
                <!-- 以前的路由跳转，从A路由跳转到B路由，这里加入购物车，进行路由跳转之前，
...
export default {
  ...
  methods: {
    ...
    changeCount(event) {
      ...
      //获取用户输入进来的内容||两种特殊情况（出现汉字、英文字母、特殊符号）(*1为NAN)、出现小于1的数字
    ...
    async addShopCart() {
      ...
      try {
        ...
        //不书写await失败也有可能走这里
        // 因为服务器没有返回多余的数据，以此不需要三连环仓库存储数据，只有code==200
        ...
        //本地存储或者会话存储不能存储对象--->存储字符串sessionStorage.setItem("SKUINFO"，this.skuInfo);不允许存对象，需要转化为字符串
        // sessionStorage.setItem("SKUINFO", JSON.stringify(this.skuInfo));
        // 拿取时需要转化为字符串JSON.parse(sessionStorage.getItem("SKUINFO"));
        // 本地存储:持久，有限制，5M。会话存储：暂时，会话结束就消失
        // localStorage
        sessionStorage.setItem("SKUINFO", JSON.stringify(this.skuInfo));
        //添加购物车成功，路由跳转,需要带数据，params,query
        this.$router.push({
          name: "addcartsuccess",
